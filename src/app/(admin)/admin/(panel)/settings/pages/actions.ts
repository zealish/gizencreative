...
import {
  BENTO_CUSTOM_IMAGE_KEY,
  CTA_DARK_IMAGE_KEY,
  USE_CASE_IDS,
  USE_CASE_IMAGE_PREFIX,
} from "@/lib/settings";
...
export async function saveBentoImage(formData: FormData) {
  ...
  const parsed = imageUrlSchema.safeParse(formData.get("bento-custom") ?? "");
  if (!parsed.success) {
    throw new Error("Invalid image URL for bento custom card");
  }
  await db
    .insert(siteSetting)
    .values({
      key: BENTO_CUSTOM_IMAGE_KEY,
      value: parsed.data,
      updatedAt: new Date(),
    })
    .onConflictDoUpdate({
...
export async function saveCtaDarkImage(formData: FormData) {
  ...
  const parsed = imageUrlSchema.safeParse(formData.get("cta-dark") ?? "");
  if (!parsed.success) {
    throw new Error("Invalid image URL for CTA section");
  }
  await db
    .insert(siteSetting)
    .values({
      key: CTA_DARK_IMAGE_KEY,
      value: parsed.data,
      updatedAt: new Date(),
    })
    .onConflictDoUpdate({
