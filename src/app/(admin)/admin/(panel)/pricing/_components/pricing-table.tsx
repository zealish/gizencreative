...
import { pricingServices } from "@/lib/pricing-shared";
...
function RowActionsMenu({ plan }: { plan: PricingRow }) {
...
export function PricingTable({ plans }: { plans: PricingRow[] }) {
  const t = useTranslations("pricingAdmin");
  const columns: ColumnDef<Features, PricingRow, unknown>[] = [
    {
      accessorKey: "nameId",
      header: ({ column }) => (
        <DataTableColumnHeader
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("service")}
          key={`service-${column.getIsSorted()}`}
        />
      ),
      cell: ({ row }) => (
        <span>
          {t(`services.${row.original.service}`)}
          {row.original.seoTier
            ? ` · ${t(`seoTiers.${row.original.seoTier}`)}`
            : ""}
        </span>
      ),
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("price")}
          key={`price-${column.getIsSorted()}`}
        />
      ),
      cell: ({ row }) => row.original.price ?? t("customPrice"),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("sortOrder")}
          key={`sortOrder-${column.getIsSorted()}`}
        />
      ),
      meta: { className: "hidden sm:table-cell" },
    },
    {
      ...
      cell: ({ row }) => (
        <span
          className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${row.original.published ? "bg-accent-soft text-accent" : "bg-black/5 text-muted dark:bg-white/10"}`}
        >
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => <RowActionsMenu plan={row.original} />,
    },
  ];
  const filters: DataTableFilter[] = [
    {
      columnId: "service",
      title: t("service"),
      options: pricingServices.map((service) => ({
        value: service,
