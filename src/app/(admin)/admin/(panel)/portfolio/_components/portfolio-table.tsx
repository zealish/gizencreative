...
function RowActionsMenu({ post }: { post: PortfolioRow }) {
  const t = useTranslations("portfolioAdmin");
...
export function PortfolioTable({ projects }: { projects: PortfolioRow[] }) {
  const t = useTranslations("portfolioAdmin");
  const columns: ColumnDef<Features, PortfolioRow, unknown>[] = [
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t("projectName")}
          key={`projectName-${column.getIsSorted()}`}
        />
      ),
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          ...
          key={`clientCompany-${column.getIsSorted()}`}
        />
      ),
